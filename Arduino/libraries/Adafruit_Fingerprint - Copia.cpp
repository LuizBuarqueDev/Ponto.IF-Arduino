...
/**************************************************************************/
/*!
    @brief  Exporta template do buffer do sensor para o Arduino
    @param  templateBuffer ponteiro para o array que vai receber os bytes
    @param  buffSize tamanho esperado do template (512 ou 768, depende do sensor)
    @return FINGERPRINT_OK se sucesso, ou código de erro
*/
/**************************************************************************/
int Adafruit_Fingerprint::downloadModel(uint8_t *templateBuffer, uint16_t buffSize) {
  // pede exportação para o buffer 1
  uint8_t cmd[] = {FINGERPRINT_UPCHAR, 0x01};
  writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(cmd), cmd);

  uint16_t count = 0;
  while (count < buffSize) {
    uint8_t reply[64];   // buffer local
    uint8_t packetLen = getReply(reply, 2000);
    if (!packetLen) return FINGERPRINT_PACKETRECIEVEERR;

    if (reply[0] == FINGERPRINT_DATAPACKET || reply[0] == FINGERPRINT_ENDDATAPACKET) {
      for (uint16_t i = 0; i < packetLen - 2; i++) {
        if (count < buffSize) {
          templateBuffer[count++] = reply[i];
        }
      }
      if (reply[0] == FINGERPRINT_ENDDATAPACKET) break;
    } else {
      return FINGERPRINT_BADPACKET;
    }
  }
...
/**************************************************************************/
/*!
    @brief  Envia template do Arduino para o sensor
    @param  templateBuffer ponteiro para array com bytes do template
    @param  buffSize tamanho do template (512 ou 768)
    @return FINGERPRINT_OK se sucesso, ou código de erro
*/
/**************************************************************************/
int Adafruit_Fingerprint::uploadModel(uint8_t *templateBuffer, uint16_t buffSize) {
  // avisa que vai enviar para buffer 1
  uint8_t cmd[] = {FINGERPRINT_DOWNCHAR, 0x01};
  writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(cmd), cmd);

  uint16_t sent = 0;
  while (sent < buffSize) {
    uint16_t chunk = (buffSize - sent > 128) ? 128 : (buffSize - sent);
    uint8_t *data = templateBuffer + sent;

    uint8_t packetType = (sent + chunk >= buffSize) ? FINGERPRINT_ENDDATAPACKET : FINGERPRINT_DATAPACKET;
    writePacket(theAddress, packetType, chunk, data);
  ...
  uint8_t packetLen = getReply(reply, 2000);
  if (!packetLen) return FINGERPRINT_PACKETRECIEVEERR;
  if (reply[0] != FINGERPRINT_ACKPACKET) return FINGERPRINT_BADPACKET;
  if (reply[1] != FINGERPRINT_OK) return reply[1];
